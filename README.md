# 🍴 KT광화문빌딩 맛집 지도 (Food Map)

KT광화문빌딩 주변 맛집을 **카테고리별로** 모아보고, 카카오맵에 마커로 표시하며,
...
| `NEXT_PUBLIC_COMPANY_NAME` | (선택) KT광화문빌딩 이름 — 기준 마커 라벨. 기본 `KT광화문빌딩East` |
| `NEXT_PUBLIC_COMPANY_ADDRESS` | (선택) KT광화문빌딩 주소 |
| `NEXT_PUBLIC_COMPANY_LAT` | (선택) KT광화문빌딩 위도 — 지도 기준점. 기본 KT광화문빌딩East |
| `NEXT_PUBLIC_COMPANY_LNG` | (선택) KT광화문빌딩 경도 |
...
git init
git add .
git commit -m "feat: KT광화문빌딩 맛집 지도 초기 버전"
# GitHub 저장소 생성 후
...
- KT광화문빌딩 위치: `.env.local`의 `NEXT_PUBLIC_COMPANY_LAT/LNG`
